...
export const apiClient = {
  ...
  getLeaderboardMe: async (params?: { courseId?: string }) => {
    const queryParams = new URLSearchParams()
    if (params?.courseId) queryParams.append('courseId', params.courseId)

    const queryString = queryParams.toString() ? `?${queryParams.toString()}` : ''
    const res = await fetch(`${API_BASE_URL}/api/leaderboard/me${queryString}`, {
      method: 'GET',
      headers: getAuthHeaders(),
    })
    return res.json()
  ...
  getLeaderboardPodium: async (params?: { courseId?: string }) => {
    const queryParams = new URLSearchParams()
    if (params?.courseId) queryParams.append('courseId', params.courseId)

    const queryString = queryParams.toString() ? `?${queryParams.toString()}` : ''
    const res = await fetch(`${API_BASE_URL}/api/leaderboard/podium${queryString}`, {
      method: 'GET',
      headers: getAuthHeaders(),
    })
    return res.json()
